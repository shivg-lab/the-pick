...
const baseOutput=resolve('../docs/demo-video-v3');
const output=rehearsal?`${baseOutput}/rehearsal`:baseOutput;
const scenes=JSON.parse(await readFile(`${baseOutput}/narration.json`,'utf8'));
...
async function scene(id){const s=await caption(id);await pause(s.audio_seconds*1000+650);}
...
async function investigate(id,expected){
 ...
 const to=seconds();waits.push({start:from,end:to,keep_start:chapter.audio_seconds+1,keep_end:2});
 await pause(Math.max(500,(chapter.audio_seconds+.8-(seconds()-from))*1000));
...
try{
 ...
 await click(page.locator('.primary-card').getByRole('button',{name:'Know before you go',exact:true}));
 await focus('.guide-body',85);await caption('iconic-guide');
 const guideStart=seconds();await pause(4000);await tab('food & arrival');
 await pause(Math.max(1200,(script['iconic-guide'].audio_seconds+.7-(seconds()-guideStart))*1000));
 await tab('parking & comfort');await focus('.preparation-section:first-child',85);
 await caption('iconic-comfort');const comfortStart=seconds();await pause(4500);
 await focus('.preparation-section:nth-child(2)',85);
 await pause(Math.max(1500,(script['iconic-comfort'].audio_seconds+.7-(seconds()-comfortStart))*1000));
 if(!(await page.locator('.weather-summary').innerText()).includes('outside the available forecast window'))throw new Error('Distant event incorrectly received a forecast.');
 await focus('.weather-summary',90);await scene('iconic-weather');
 await click(page.getByRole('button',{name:'Close guide'}));
 await focus('.alternatives-section',85);await scene('iconic-alternatives');
 await focus('.how-it-works',160);await scene('closing');await scene('disclaimer');
 contentEnd=seconds();
